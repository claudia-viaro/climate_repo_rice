...
from my_project.utils.fixed_paths import REPO_DIR, REGION_YAMLS_DIR
import numpy as np
...
_PUBLIC_REPO_DIR = str(REPO_DIR)  # convert Path to str for os.path.join
...
# This Python class represents an environment for simulating a negotiation scenario related to rice
# production with climate and economic factors.
class Rice(gym.Env):
    name = "Rice"
    ...
    def __init__(
        ...
        self.is_baseline = not relative_reward  # True if it's the BAU baseline
        if actions_masked is None:
            self.actions_masked = {}
        elif not isinstance(actions_masked, dict):
            raise ValueError(
                "actions_masked must be a dictionary of {action_name: fixed_value}"
            )
        else:
            self.actions_masked = actions_masked
            print(f"actions_masked = {self.actions_masked}")
        self.region_yamls_path = region_yamls_path
        if self.region_yamls_path is None:
            self.region_yamls_path = os.path.join(
                _PUBLIC_REPO_DIR, "my_project", "configs", "region_yamls"
            )
        ...
        # print(f"[RiceEnv] Using region_yamls_path={self.region_yamls_path}")
        ...
        # strange part about welfare multiplier loss that scales down the reward (40%? or 4%?)
        self.apply_welfloss = False
        self.apply_welfgain = False
        ...
        if relative_reward:
            ...
            self.baseline_rice = Rice(
                ...
                relative_reward=False,  # BAU baseline
                step_logger=None,
                region_yamls_path=self.region_yamls_path,
            )

            # ✅ Run BAU once and collect history
            self._run_and_store_bau()

            # Save final rewards for relative reward comparison
            self.baseline_rewards = self.baseline_rice.get_rewards()

    def _run_and_store_bau(self):
        """Run BAU once and store full trajectory in self.bau_history."""
        obs, _ = self.baseline_rice.reset()
        ...
        self.bau_history = []  # reset

        while True:
            obs, rew, done, truncated, info = self.baseline_rice.step(
                actions, skip_negotiation=True
            )

            # --- Debug: check savings ---
            if "savings" in self.actions_masked:
                fixed_savings = self.actions_masked["savings"]

                # 1️⃣ Global state check
                current_savings = self.baseline_rice.global_state[
                    "savings_all_regions"
                ]["value"][self.baseline_rice.current_timestep, :]
                for region_id, s in enumerate(current_savings):
                    if not np.isclose(s, fixed_savings):
                        print(
                            f"[DEBUG WARNING] BAU savings mismatch (global_state) "
                            f"timestep {self.baseline_rice.current_timestep}, region {region_id}: {s} != {fixed_savings}"
                        )

                # 2️⃣ Info check
                if "savings_all_regions" in info:
                    info_savings = info["savings_all_regions"]
                    for region_id, s in enumerate(info_savings):
                        if not np.isclose(s, fixed_savings):
                            print(
                                f"[DEBUG WARNING] BAU savings mismatch (info) "
                                f"timestep {self.baseline_rice.current_timestep}, region {region_id}: {s} != {fixed_savings}"
                            )

            # get step phase
            info_stage = info.get("__common__", {}).get("stage", "unknown")

            # Print rewards compactly
            rew_values = list(rew.values()) if isinstance(rew, dict) else rew
            if isinstance(rew_values, (list, np.ndarray)):
                rew_preview = np.array(rew_values)[:3]  # only first 3 regions
            else:
                rew_preview = rew_values

            activity = self.baseline_rice.global_state["activity_timestep"]["value"][
                self.baseline_rice.current_timestep
            ]

            # ✅ Collect BAU features
            step_bau = {}
            for var in bau_features:
                step_bau[var] = self.baseline_rice.get_state(var)
            step_bau["timestep"] = self.baseline_rice.global_state["activity_timestep"][
                "value"
            ][self.baseline_rice.current_timestep]
            step_bau["reward"] = rew  # store rewards too
            self.bau_history.append(step_bau)

            if done["__all__"]:
                break

        # ✅ Trim history to 20 timesteps (ignore extra runs)
    ...
    def apply_fixed_actions_to_vector(self, actions_dict):
        for action_type, fixed_value in self.actions_masked.items():
            key = f"{action_type}_all_regions"

            # 2D actions
            if action_type in ["import_bids", "import_tariffs"]:
                arr = []
                for r in range(self.num_regions):
                    if isinstance(fixed_value, (list, np.ndarray)) and r < len(
                        fixed_value
                    ):
                        row = fixed_value[r]
                        # Ensure row is a NumPy array for arithmetic
                        if isinstance(row, list):
                            row = np.array(row, dtype=self.float_dtype)
                        arr.append(row)
                    else:
                        arr.append(np.zeros(self.num_regions, dtype=self.float_dtype))
                actions_dict[key] = arr  # still a list of arrays, preserves structure

            # 1D actions
            elif action_type in ["savings", "mitigation_rate", "export_limit"]:
                if isinstance(fixed_value, (list, np.ndarray)):
                    actions_dict[key] = np.array(fixed_value, dtype=self.float_dtype)
                else:
                    actions_dict[key] = np.full(
                        self.num_regions, fixed_value, dtype=self.float_dtype
                    )
    ...
    def generate_info(self, *, seed=None, options=None):
        """
        Generate structured info for RL step, including:
        1) info: object returned by step function (full structure, common + per-agent)
        2) info_rl: RL info as before
        3) info_diffs: RL vs BAU differences for selected metrics
        """
        # --- Common temperature calculations ---
        # Get current and initial global temperature (atmospheric only, index 0)
        temp_now = self.get_state("global_temperature", timestep=self.current_timestep)[
            0
        ]
        temp_0 = self.get_state("global_temperature", timestep=0)[0]
        temp_rise_rl = temp_now - temp_0

        # BAU temperature at same timestep and negotiation stage 0
        temp_bau = self.baseline_rice.get_state(
            "global_temperature", timestep=self.current_timestep
        )[0]
        temp_0_bau = self.baseline_rice.get_state("global_temperature", timestep=0)[0]
        temp_rise_bau = temp_bau - temp_0_bau
        temp_rise_bau_rl = temp_rise_rl - temp_rise_bau  # difference RL vs BAU

        # --- Variables to include as per-region diffs ---
        # Variables for which we want per-agent diffs
        variables = [
            "utility_all_regions",
            "social_welfare_all_regions",
            "gross_output_all_regions",
        ...
        info = {
            "__common__": {
                ...
                "rl": True,
                "temp_rise_rl": temp_rise_rl,
                "temp_rise_bau_rl": temp_rise_bau_rl,
                "metrics": [f"diff_{var}" for var in variables],
        ...
        for agent in range(self.num_agents):
            ...
            for var in variables:
                rl_val = self.get_state(
                    var, region_id=agent, timestep=self.current_timestep
                )
                rl_val_0 = self.get_state(var, region_id=agent, timestep=0)
                agent_info_dict[f"diff_{var}"] = rl_val - rl_val_0
            info[agent] = agent_info_dict

        return info

    def step(self, actions, skip_negotiation=False):
        self.current_timestep += 1
        self.set_state("timestep", self.current_timestep, dtype=self.int_dtype)

        self.set_current_global_state_to_past_global_state()

        if self.negotiation_on and not skip_negotiation:
    ...
    def step_propose(self, actions=None, actions_dict_save=None):
        ...
        # Update global state
        promised_mitigation_array = np.array(actions_dict["promised_mitigation_rate"])
        self.set_state("promised_mitigation_rate", promised_mitigation_array)

        requested_mitigation_array = np.array(actions_dict["requested_mitigation_rate"])
        self.set_state("requested_mitigation_rate", requested_mitigation_array)
    ...
    def step_climate_and_economy(self, actions=None, actions_dict=None):
        ...
        # Build actions_dict if not provided
    ...
    def _get_rl_episode_info(self):
        ...
        num_timesteps = len(rl_history)
        num_features = len(rl_history[0]) - 1  # subtract timestep key

        return rl_history

    def check_info_populated(self, info, verbose=False):
        """
        Checks that an `info` object is correctly populated.

        Correctly populated means:
        - "__common__" contains "temp_rise" and it is not None
        - Metrics are present for each agent and have actual values (not empty lists)
        - accumulated_obs has at least one timestep for each tracked feature

        Returns True if populated, False otherwise.
        """
        if "__common__" not in info:
            if verbose:
                print("⚠️ Missing '__common__' key")
            return False

        common = info["__common__"]

        # Check temp_rise
        temp_rise = common.get("temp_rise", None)
        if temp_rise is None or (
            isinstance(temp_rise, (list, np.ndarray))
            and all(v is None for v in temp_rise)
        ):
            if verbose:
                print(f"⚠️ temp_rise missing or all None: {temp_rise}")
            return False

        # Check accumulated_obs
        accumulated_obs = common.get("accumulated_obs", None)
        if accumulated_obs is not None:
            if isinstance(accumulated_obs, list) and len(accumulated_obs) == 0:
                if verbose:
                    print("⚠️ accumulated_obs is empty list")
                return False
            if isinstance(accumulated_obs, dict):
                for agent_id, features in accumulated_obs.items():
                    for feat, ts in features.items():
                        if len(ts) == 0:
                            if verbose:
                                print(f"⚠️ accumulated_obs[{agent_id}][{feat}] is empty")
                            return False

        # Check agent-level metrics
        num_regions = common.get("num_regions", 0)
        metrics = common.get("metrics", [])
        for agent_id in range(num_regions):
            agent_data = info.get(agent_id, None)
            if agent_data is None:
                if verbose:
                    print(f"⚠️ Missing agent {agent_id} info")
                return False
            for metric in metrics:
                val = agent_data.get(metric, None)
                if val is None or (isinstance(val, list) and len(val) == 0):
                    if verbose:
                        print(
                            f"⚠️ Agent {agent_id}, metric '{metric}' missing or empty: {val}"
                        )
                    return False
    ...
    def _get_climate_episode_info(self):
        ...
        Aligned by activity timestep (so BAU and RL compare the same years).
        """

        episode_length = self.episode_length + 1
        climate_history = []

        # --- Reference values at timestep 0 (RL and BAU) ---
        ...
        # --- Iterate over RL timesteps ---
        for t in range(episode_length):
            # only climate stage timesteps
            ...
            # --- Global temp rise (RL, BAU, and difference) ---
        ...
        # ✅ Debug print summary
        if climate_history:
            num_timesteps = len(climate_history)
            num_features = (
                len(climate_history[0]) - 2
            )  # subtract timestep + activity_timestep
        else:
            print("[DEBUG] No climate timesteps found for episode")
    ...
    def calc_capitals(self, investments, save_state=True):
        ...
        capital_depreciations = np.zeros(self.num_regions, dtype=self.float_dtype)
        prev_capitals = np.zeros(self.num_regions, dtype=self.float_dtype)
        investment_terms = np.zeros(self.num_regions, dtype=self.float_dtype)
        for region_id in range(self.num_regions):
            regional_params = self.all_regions_params[region_id]
            x_delta_k = regional_params["xdelta_K"]
            x_delta = regional_params["xDelta"]
            capital_depreciation = pow(1 - x_delta_k, x_delta)
            capital_depreciations[region_id] = capital_depreciation
            prev_capital = self.get_prev_state(
                "capital_all_regions", region_id=region_id
            )
            prev_capitals[region_id] = prev_capital
            invest_term = x_delta * investments[region_id]
            investment_terms[region_id] = invest_term
            ...
            capitals[region_id] = capital_depreciation * prev_capital + invest_term
            if save_state:
                self.set_state(
    ...
    def calc_rewards(self, utilities, welfloss_multipliers, save_state=True):
        ...
        # Debug: check multipliers
        if not np.allclose(welfloss_multipliers, 1):
            print(
                f"[DEBUG WARNING] welfloss_multipliers not all 1: {welfloss_multipliers}"
            )

        for region_id in range(self.num_regions):
            # Only print for first two regions
            do_print = region_id == 0
            ...
            if not self.relative_reward:
                ...
                # ----- RELATIVE REWARD CASE -----
                # Compute RL reward minus BAU baseline
                else:
                    bau_activity_reward = self.baseline_rice.global_state[
                        "reward_all_regions"
                    ]["value"][self.activity_timestep, region_id]
                    bau_current_reward = self.baseline_rice.global_state[
                        "reward_all_regions"
                    ]["value"][self.current_timestep, region_id]
                    rewards[region_id] = (
                        utilities[region_id] * welfloss_multipliers[region_id]
    ...
    def calc_utilities(self, consumptions, save_state=True):
        ...
        for region_id in range(self.num_regions):
            ...
            labor = self.get_prev_state("labor_all_regions", region_id)
            labor_thousand = labor / 1000.0
            consumption = consumptions[region_id]
            alpha = regional_params["xalpha"]

            ratio = consumption / labor_thousand + _SMALL_NUM
            power = pow(ratio, 1 - alpha)
            utility = labor_thousand * (power - 1) / (1 - alpha)

            utilities[region_id] = utility
            if save_state:
                self.set_state(
                    "utility_all_regions",
                    utilities[region_id],
        ...
        for region_id in range(self.num_regions):
            ...
            if self.dmg_function == "base":
                ...
                damages[region_id] = (
                    ...
                    1
                    - (0.7438 * ((prev_atmospheric_temperature - 0.115) ** 2)) / 100
                )
            else:
    ...
    def calc_global_carbon_mass(self, productions, save_state=True):
        if self.carbon_model == "base":
            ...
            # Update global_emissions (this was missing before!)
            if save_state:
                self.set_state("global_emissions", sum_aux_m)
    ...
    def calc_action_mask(self):
        ...
        Includes:
        - action_window restrictions
        - minimum mitigation rate enforcement
        - permanent exclusions from YAML
        - stage-dependent masking (proposal/evaluation/economy)
        """
        mask_dict = {region_id: None for region_id in range(self.num_regions)}

        for region_id in range(self.num_regions):
            # Start from windowed or default mask
            if self.action_window:
                mask = self.calc_action_window(region_id)
            else:
                mask = self.default_agent_action_mask.copy()

            # --- Existing logic: mitigation constraints ---
            if self.negotiation_on:
            ...
            # --- Existing logic: permanent exclusions from YAML ---
            for action_type in self.actions_masked.keys():
                start, end = self.get_mask_index(action_type)
                mask[start:end] = 0

            # --- NEW logic: negotiation stage dependent masking ---
            if self.negotiation_on:
                stage_mask = np.zeros_like(mask)

                stage = int(
                    self.global_state["negotiation_stage"]["value"][
                        self.current_timestep
                    ]
                )

                if stage == 0:  # Proposal stage
                    start = self.get_actions_index("proposal")
                    end = self.get_actions_index("proposal_decisions")
                    stage_mask[start:end] = 1

                elif stage == 1:  # Evaluation stage
                    start = self.get_actions_index("proposal_decisions")
                    end = start + len(self.evaluation_possible_actions)
                    stage_mask[start:end] = 1

                else:  # Economy-climate stage
                    proposal_start = self.get_actions_index("proposal")
                    stage_mask[:proposal_start] = (
                        1  # keep economy actions, mask out proposals
                    )

                # Intersect negotiation-stage mask with existing mask
    ...
    def get_action_labels(self):
        """
        Return a list of action labels corresponding to each action index.
        Example: ["savings_0", "savings_1", ..., "mitigation_0", ..., "export_limit_0", ...]
        """
        labels = []

        def label_range(base, count):
            return [f"{base}_{i}" for i in range(count)]

        labels += label_range("savings", sum(self.savings_possible_actions))
        labels += label_range("mitigation", sum(self.mitigation_rate_possible_actions))
        labels += label_range("export_limit", sum(self.export_limit_possible_actions))
        labels += label_range("import_bids", sum(self.import_bids_possible_actions))
        labels += label_range(
            "import_tariffs", sum(self.import_tariff_possible_actions)
        )
        labels += label_range("proposal", sum(self.proposal_possible_actions))
        labels += label_range("evaluation", sum(self.evaluation_possible_actions))
    ...
    def get_incoming_accepted_mitigation_rates(self, region_id):
        ...
        # print(f"Incoming accepted mitigation for region {region_id}:", incoming)
        return incoming
    ...
    def get_actions(self, action_type, actions):
        # Use fixed value if action_type is masked
        if action_type in self.actions_masked:
            fixed_value = self.actions_masked[action_type]
            if isinstance(fixed_value, (list, np.ndarray)):
                return list(fixed_value)
            else:
                return [fixed_value] * self.num_regions
    ...
    def set_all_region_params(self):
        param_path = self.region_yamls_path
        # print(f"[RiceEnv] Reading region YAMLs from: {param_path}")
        assert os.path.exists(
            os.path.join(param_path, "default.yml")
        ), f"Missing default.yml in {param_path}"
    ...
    def set_default_agent_action_mask(self):
        ...
        if self.actions_masked:
            for action_name, fixed_value in self.actions_masked.items():
                mask_start, mask_end = self.get_mask_index(action_name)
                self.default_agent_action_mask[mask_start:mask_end] = 0
                # Also store the fixed value for later use
                if not hasattr(self, "fixed_action_values"):
                    self.fixed_action_values = {}
                self.fixed_action_values[action_name] = fixed_value
    ...
    def get_observations(self):
        ...
        for feature in global_features + public_features:
            ...
            feature_keys.extend([feature] * len(flattened))
        ...
        for region_id in range(self.num_regions):
            ...
            region_keys = [f"region_{i}_indicator" for i in range(self.num_regions)]
            region_feature_keys = (
                region_keys + feature_keys.copy()
            )  # Start with indicators + shared
            ...
            for feature in private_features:
                ...
                region_feature_keys.extend([feature] * len(flattened))

            for feature in bilateral_features:
                assert self.global_state[feature]["value"].shape[1] == self.num_regions
                assert self.global_state[feature]["value"].shape[2] == self.num_regions

                out_flat = self.flatten_array(
                    self.global_state[feature]["value"][
                        self.current_timestep, region_id
                    ]
                    / self.global_state[feature]["norm"]
                )
                in_flat = self.flatten_array(
                    self.global_state[feature]["value"][
                        self.current_timestep, :, region_id
                    ]
                    / self.global_state[feature]["norm"]
                )
                all_features = np.append(all_features, out_flat)
                all_features = np.append(all_features, in_flat)

                region_feature_keys.extend([f"{feature}_to_others"] * len(out_flat))
                region_feature_keys.extend([f"{feature}_from_others"] * len(in_flat))

            features_dict[region_id] = all_features
            feature_key_dict[region_id] = region_feature_keys
    ...
    def make_step_debug(self, global_state, t, num_regions):
        """
        Collects a structured debug snapshot at timestep t.
        Designed to be compact and easy for both timeseries and longitudinal analysis.
        """

        # ---- Global features
        global_features = {
            "global_emissions": (
                global_state["global_emissions"]["value"][t].item()
                if global_state["global_emissions"]["value"].ndim == 1
                else global_state["global_emissions"]["value"][t].tolist()
            )
        }

        # ---- Public features
        public_features = {
            "capital": global_state["capital_all_regions"]["value"][t].tolist(),
            "gross_output": global_state["gross_output_all_regions"]["value"][
                t
            ].tolist(),
            "investment": global_state["investment_all_regions"]["value"][t].tolist(),
            "savings": global_state["savings_all_regions"]["value"][t].tolist(),
            "mitigation_rates": global_state["mitigation_rates_all_regions"]["value"][
                t
            ].tolist(),
        }
        ...
        for f in private_vars:
            private_features[f] = global_state[f]["value"][t, :num_regions].tolist()

        # ---- Bilateral features
        bilateral_features = {
            "promised_mitigation_rate": global_state["promised_mitigation_rate"][
                "value"
            ][t].tolist(),
            "requested_mitigation_rate": global_state["requested_mitigation_rate"][
                "value"
            ][t].tolist(),
            "proposal_decisions": global_state["proposal_decisions"]["value"][
                t
            ].tolist(),
        }
    ...
    def get_last_episode_rewards(self):
        """
        Return rewards at the last timestep of the episode (when current_simulation_year == end_year).
        """
        rewards = {}
        key = "reward_all_regions"

        if key not in self.global_state:
            print("[DEBUG] Reward key missing, returning zeros")
            return {f"region_{i}": 0.0 for i in range(self.num_regions)}

        reward_array = self.global_state[key]["value"]
        # The last timestep is simply the last index along the timestep dimension
        last_timestep_rewards = reward_array[-1]

        for region_id in range(self.num_regions):
            rewards[f"region_{region_id}"] = float(last_timestep_rewards[region_id])
    ...
    def set_state(
        ...
        if isinstance(value, list):
            ...
            if key == "minimum_mitigation_rate_all_regions":
                # For this key, keep scalar as scalar (do not wrap)
                pass
            else:
                # For other keys, wrap scalar as 1-element array for consistency
                value = np.array([value], dtype=dtype)
        else:
        ...
        if key not in self.global_state:
            ...
            if region_id is None:
                shape = (self.episode_length + 1,) + value.shape
                self.global_state[key] = {
                    "value": np.zeros(shape, dtype=dtype),
                    "norm": norm,
                }
            else:
                shape = (self.episode_length + 1, self.num_regions) + value.shape
                self.global_state[key] = {
                    "value": np.zeros(shape, dtype=dtype),
                    "norm": norm,
                }
